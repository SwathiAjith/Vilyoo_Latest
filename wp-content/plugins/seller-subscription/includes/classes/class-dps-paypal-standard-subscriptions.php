...
class DPS_PayPal_Standard_Subscriptions {
    ...
    private static function log( $message = '' ) {
        $settings = self::get_wc_paypal_settings();
    ...
    public static function paypal_settings_args( $form_fields ) {
        ...
        $form_fields['email']['description'] .= ' </p><p class="description">' . __( 'It is <strong>strongly recommended you do not change this email address</strong> if you have active subscriptions with PayPal. Doing so can break existing subscriptions.', 'dps' );

        $form_fields += array(
            'api_credentials' => array(
                'title'       => __( 'API Credentials', 'dps' ),
                'type'        => 'title',
                'description' => sprintf( __( 'Enter your PayPal API credentials to unlock subscription suspension and cancellation features. %sLearn More &raquo;%s', 'dps' ), '<a href="http://docs.woothemes.com/document/store-manager-guide/#extrapaypalconfigurationsteps" target="_blank" tabindex="-1">', '</a>' ),
            ),
            'api_username' => array(
                'title'       => __( 'API Username', 'dps' ),
                'type'        => 'text',
                'description' => '',
                'default'     => ''
            ),
            'api_password' => array(
                'title'       => __( 'API Password', 'dps' ),
                'type'        => 'text',
                'description' => '',
                'default'     => ''
            ),
            'api_signature' => array(
                'title'       => __( 'API Signature', 'dps' ),
                'type'        => 'text',
                'description' => '',
                'default'     => ''
            )
    ...
    public static function paypal_standard_subscription_args( $paypal_args ) {
        ...
        $custom      = maybe_unserialize( $paypal_args['custom'] );
        $order_id    = $custom[0];
        $order_key   = $custom[1];
        ...
        if ( ! Dokan_Product_Subscription::is_subscription_product( $product->id ) ) {
            return $paypal_args;
        }

        if ( get_post_meta( $product->id, '_enable_recurring_payment', true ) == 'no' ) {
            return $paypal_args;
        }
        ...
        $unconverted_periods = array(
            'billing_period' => get_post_meta( $product->id, '_subscription_period', true )
        );
        ...
        $subscription_interval     = get_post_meta( $product->id, '_subscription_period_interval', true );
        $subscription_installments = get_post_meta( $product->id, '_subscription_length', true );
    ...
    public static function process_paypal_ipn_request( $transaction_details ) {
        ...
        $custom    = maybe_unserialize( $transaction_details['custom'] );
        $order_id  = $custom[0];
        $order_key = $custom[1];
        ...
        switch ($transaction_details['txn_type']) {
            ...
            case 'subscr_payment':
                ...
                if ( 'completed' == strtolower( $transaction_details['payment_status'] ) ) {
                    ...
                    $order->add_order_note( sprintf( __( 'IPN subscription payment completed. txn_id#%s', 'dps' ), $transaction_details['txn_id'] ) );

                } elseif ( 'failed' == strtolower( $transaction_details['payment_status'] ) ) {

                    // Subscription Payment completed
                    $order->add_order_note( __( 'IPN subscription payment failed.', 'dps' ) );
            ...
            case 'subscr_cancel':
                ...
                // Subscription Payment completed
                $order->add_order_note( __( 'IPN subscription cancelled for order.', 'dps' ) );

                Dokan_Product_Subscription::delete_subscription_pack( $customer_id );
            ...
            case 'subscr_eot': // Subscription ended, either due to failed payments or expiration
                ...
                if ( 1 != $subscription_length && $subscription_length != get_post_meta( $product['product_id'], '_subscription_period_interval', true ) ) {
                    ...
                    $order->add_order_note( __( 'IPN subscription end-of-term for order.', 'dps' ) );
            ...
            case 'subscr_failed': // Subscription sign up failed
                ...
                // Subscription Payment completed
                $order->add_order_note( __( 'IPN subscription payment failure.', 'dps' ) );
    ...
    public static function cancel_subscription_with_paypal( $order_id, $user_id ) {
        ...
        $order        = new WC_Order( $order_id );
        $profile_id   = get_post_meta( $order->id, '_paypal_subscriber_ID', true );
        ...
        if ( $response ) {
            ...
            Dokan_Product_Subscription::delete_subscription_pack( $user_id );

            $order->add_order_note( __( 'Subscription cancelled with PayPal', 'dps' ) );
        }
    }
    ...
    public static function change_subscription_status( $profile_id, $new_status ) {
        ...
        switch ($new_status) {
            case 'Cancel' :
                $new_status_string = __( 'cancelled', 'dps' );
                break;
            case 'Suspend' :
                $new_status_string = __( 'suspended', 'dps' );
                break;
            case 'Reactivate' :
                $new_status_string = __( 'reactivated', 'dps' );
                break;
        }
