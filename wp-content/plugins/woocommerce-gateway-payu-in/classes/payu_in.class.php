...
class WC_Gateway_Payu_In extends WC_Payment_Gateway {
	...
	*/
	function init_form_fields() {
		$this->form_fields = array(
			...
			'merchantid' => array(
							...
							'type' => 'text',
							'description' => __( 'Please enter the Merchant Key provided by PayUMoney', 'woocommerce_payu_in' ),
							'default' => ''
						),
			'salt' => array(
							'title' => __( 'SALT', 'woocommerce_payu_in' ),
							'type' => 'text',
							'description' => __( 'Please enter the Salt provided by PayUMoney', 'woocommerce_payu_in' ),
							'default' => ''
						),
	...
	public function admin_options() {
		...
		?>
		<div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'woocommerce_payu_in' ); ?></strong> <?php echo sprintf( __( 'Choose Indian Rupee (₹) as your store currency in <a href="%s">Pricing Options</a> to enable the PayUMoney Gateway.', 'woocommerce_payu_in' ), admin_url( '?page=woocommerce&tab=general' ) ); ?></p></div>
		<?php
		} // End check currency
	} // End admin_options()
    ...
    function generate_payu_in_form($order_id) {
		...
		$payu_in_args = array(
			...
			'curl'					=> add_query_arg(array('payu_in_callback' => 1, 'payu_in_status' => 'cancelled'), $this->get_return_url( $order)),
		...
		return '<form action="' . $posturl . '" method="POST" name="payform" id="payform">
				...
				<script type="text/javascript">
					jQuery(function(){
						jQuery("body").block(
							{
								message: "<img src=\"' . $woocommerce->plugin_url() . '/assets/images/ajax-loader.gif\" alt=\"Redirecting...\" />'.__('Thank you for your order. We are now redirecting you to PayUMoney to make payment.', 'woothemes').'",
								overlayCSS:
								{
									background: "#fff",
									opacity: 0.6
	...
	function receipt_page($order) {
		echo '<p>' . __( 'Thank you for your order, please click the button below to pay with PayUMoney.', 'woocommerce_payu_in' ) . '</p>';
	...
	function payment_pending($txnid) {
			...
			$order->add_order_note(sprintf( __( 'PayUMoney payment pending (Transaction id: %s)', 'woocommerce_payu_in' ), $txnid ));
	...
	function payment_failure($txnid) {
		global $woocommerce;
			...
			$order->add_order_note(sprintf( __( 'PayUMoney payment failed (Transaction id: %s)', 'woocommerce_payu_in' ), $txnid ));
